...
if not exist "%FFMPEG%" (
    ...
    echo   Rode %WHITE%1_SETUP.bat%RST% para instalar o conversor completo.
    echo.
    pause
    endlocal
    exit /b 1
...
if errorlevel 1 (
    ...
    echo   Rode %WHITE%1_SETUP.bat%RST% e aceite a instalacao opcional do Calibre.
    echo.
    pause
    goto :category
)
...
echo   %WHITE%[3]%RST% Abrir data/logs
echo   %YELLOW%[B]%RST% Voltar
echo.
set "SOPT="
...
if "!SOPT!"=="3" start "" explorer.exe "%~dp0data"
