...
call :init_app
call "%~dp0omni-ui.bat" splash "OMNITOOLS" "arsenal local para midia e produtividade"
...
echo   %RED%[erro]%RST% ffmpeg.exe nao encontrado. Rode Instalar.bat.
pause
exit /b 1

:need_ffprobe
if exist "%FFPROBE%" exit /b 0
echo   %RED%[erro]%RST% ffprobe.exe nao encontrado. Rode Instalar.bat.
pause
exit /b 1

:need_ytdlp
if exist "%YTDLP%" exit /b 0
echo   %RED%[erro]%RST% yt-dlp.exe nao encontrado. Rode Instalar.bat.
pause
exit /b 1
...
:init_app
if not exist "%~dp0..\data" mkdir "%~dp0..\data" >nul 2>&1
if not exist "%~dp0..\logs" mkdir "%~dp0..\logs" >nul 2>&1
if not exist "%TOOLS_OUT%" mkdir "%TOOLS_OUT%" >nul 2>&1
if exist "%~dp0ensure-config.bat" call "%~dp0ensure-config.bat" "%~dp0.."
exit /b
